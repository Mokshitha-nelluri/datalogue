...
function formatSchemaJSON(tables: TableInfo[]): string {
  const schema: Record<string, string[]> = {};
  for (const table of tables) {
    schema[table.name] = table.columns.map(
      (c) =>
        `${c.name} (${c.type}${c.nullable ? ', nullable' : ''}${c.isPrimaryKey ? ', PK' : ''}${c.isForeignKey && c.references ? `, FK → ${c.references.table}.${c.references.column}` : ''})`,
    );
  }
...
export function buildPrompt(
  ...
  const filteredTables = schema.tables.filter((t) =>
    allowedTables.map((n) => n.toLowerCase()).includes(t.name.toLowerCase()),
  );
