 ...
 */
export function enforceAllowlist(referencedTables, allowedTables) {
    ...
    for (const table of referencedTables) {
        ...
        if (!allowedLower.has(name.toLowerCase())) {
            throw new DatalogueError(`Table "${table}" is not in the allowed tables list`, 'TABLE_NOT_ALLOWED');
        }
    }
}
