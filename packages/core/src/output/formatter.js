 ...
 */
export function generateChartSpec(rows) {
    if (!rows.length)
        return null;
    const columns = Object.keys(rows[0]);
    const numericColumns = columns.filter((col) => rows.every((row) => typeof row[col] === 'number' || !isNaN(Number(row[col]))));
    ...
    const isTimeSeriesByValue = rows.length > 1 &&
        rows.every((row) => {
    ...
    if (isTimeSeries)
        type = 'line';
    else if (hasPercentage && isSmallSet)
        type = 'pie';
    return {
        type,
        data: {
 ...
 */
export function formatQueryResult(rows, sql, summary, confidence, executionTimeMs, outputFormats) {
    const result = {
        sql,
        rows,
        summary,
