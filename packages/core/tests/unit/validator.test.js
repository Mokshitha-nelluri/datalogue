...
import { validateSQL, stripSQLComments, } from '../../src/security/validator.js';
const baseOpts = {
...
describe('validateSQL — valid queries', () => {
    ...
    it('accepts SELECT with JOIN', () => {
        const result = validateSQL('SELECT o.id, c.name FROM orders o JOIN customers c ON o.customer_id = c.id', baseOpts);
        expect(result.valid).toBe(true);
    });
    it('accepts SELECT with subquery', () => {
        const result = validateSQL('SELECT * FROM orders WHERE total > (SELECT AVG(total) FROM orders)', baseOpts);
        expect(result.valid).toBe(true);
    });
    it('accepts SELECT with WHERE clause', () => {
        const result = validateSQL("SELECT id FROM customers WHERE name = 'Alice'", baseOpts);
        expect(result.valid).toBe(true);
    });
...
describe('validateSQL — blocked statement types', () => {
    ...
    it('blocks ALTER TABLE', () => {
        const result = validateSQL('ALTER TABLE orders ADD COLUMN new_col text', baseOpts);
        expect(result.valid).toBe(false);
        expect(result.reason).toContain('destructive statement type');
    });
    it('blocks CREATE TABLE', () => {
        const result = validateSQL('CREATE TABLE evil (id int)', baseOpts);
        expect(result.valid).toBe(false);
        expect(result.reason).toContain('destructive statement type');
    });
    ...
    it('blocks ALTER even with allowMutations: true', () => {
        const result = validateSQL('ALTER TABLE orders ADD COLUMN x text', { ...baseOpts, allowMutations: true });
        expect(result.valid).toBe(false);
    });
});
// ─── Mutation control ────────────────────────────────────────────────────────
describe('validateSQL — mutation control', () => {
    it('blocks INSERT when allowMutations is false', () => {
        const result = validateSQL("INSERT INTO orders (id, total) VALUES (1, 100)", baseOpts);
        expect(result.valid).toBe(false);
        expect(result.reason).toBe('MUTATION_NOT_ALLOWED');
    });
    it('blocks DELETE when allowMutations is false', () => {
        const result = validateSQL('DELETE FROM orders WHERE id = 1', baseOpts);
        expect(result.valid).toBe(false);
        expect(result.reason).toBe('MUTATION_NOT_ALLOWED');
    });
    it('blocks UPDATE when allowMutations is false', () => {
        const result = validateSQL('UPDATE orders SET total = 200 WHERE id = 1', baseOpts);
        expect(result.valid).toBe(false);
        expect(result.reason).toBe('MUTATION_NOT_ALLOWED');
    });
    it('allows INSERT when allowMutations is true', () => {
        const result = validateSQL("INSERT INTO orders (id, total) VALUES (1, 100)", { ...baseOpts, allowMutations: true });
        expect(result.valid).toBe(true);
    });
    ...
    it('allows UPDATE when allowMutations is true', () => {
        const result = validateSQL('UPDATE orders SET total = 200 WHERE id = 1', { ...baseOpts, allowMutations: true });
        expect(result.valid).toBe(true);
    });
});
...
describe('validateSQL — SQL injection', () => {
    it('blocks multi-statement injection (semicolon)', () => {
        const result = validateSQL('SELECT * FROM orders; DROP TABLE orders', baseOpts);
        expect(result.valid).toBe(false);
        expect(result.reason).toBe('SQL_INJECTION_BLOCKED');
    });
    it('blocks UNION-based injection into system tables', () => {
        const result = validateSQL('SELECT id FROM orders UNION SELECT tablename FROM pg_catalog.pg_tables', baseOpts);
        expect(result.valid).toBe(false);
    ...
    it('blocks hex-encoded strings', () => {
        const result = validateSQL('SELECT * FROM orders WHERE name = 0x44524F50', baseOpts);
        expect(result.valid).toBe(false);
    ...
    it('strips comments before validation', () => {
        ...
        const result = validateSQL('SELECT id FROM orders -- this is a comment', baseOpts);
        expect(result.valid).toBe(true);
    });
    it('strips block comments', () => {
        const result = validateSQL('SELECT id FROM orders /* secret injection */', baseOpts);
        expect(result.valid).toBe(true);
    });
...
describe('validateSQL — system schema access', () => {
    it('blocks pg_catalog access', () => {
        const result = validateSQL('SELECT * FROM pg_catalog.pg_tables', baseOpts);
        expect(result.valid).toBe(false);
        expect(result.reason).toContain('system schema access blocked');
    });
    it('blocks information_schema access', () => {
        const result = validateSQL('SELECT * FROM information_schema.columns', baseOpts);
        expect(result.valid).toBe(false);
        expect(result.reason).toContain('system schema access blocked');
    });
});
...
describe('validateSQL — dangerous functions', () => {
    it('blocks pg_read_file', () => {
        const result = validateSQL("SELECT pg_read_file('/etc/passwd')", baseOpts);
        expect(result.valid).toBe(false);
        expect(result.reason).toContain('pg_read_file');
    });
    it('blocks lo_import', () => {
        const result = validateSQL("SELECT lo_import('/etc/passwd')", baseOpts);
        expect(result.valid).toBe(false);
        expect(result.reason).toContain('lo_import');
    });
    it('blocks xp_cmdshell', () => {
        const result = validateSQL("EXEC xp_cmdshell 'dir'", { ...baseOpts, dialect: 'MSSQL' });
        expect(result.valid).toBe(false);
        expect(result.reason).toContain('xp_cmdshell');
    });
    it('blocks LOAD_FILE (MySQL)', () => {
        const result = validateSQL("SELECT load_file('/etc/passwd')", { ...baseOpts, dialect: 'MySQL' });
        expect(result.valid).toBe(false);
        expect(result.reason).toContain('load_file');
    });
    it('blocks load_extension (SQLite)', () => {
        const result = validateSQL("SELECT load_extension('./evil.so')", { ...baseOpts, dialect: 'SQLite' });
        expect(result.valid).toBe(false);
        expect(result.reason).toContain('load_extension');
    });
    it('blocks dblink', () => {
        const result = validateSQL("SELECT * FROM dblink('host=evil.com dbname=test', 'SELECT 1')", baseOpts);
        expect(result.valid).toBe(false);
...
describe('validateSQL — table allowlist', () => {
    ...
    it('checks all tables in joins', () => {
        const result = validateSQL('SELECT * FROM orders o JOIN secret_data s ON o.id = s.order_id', baseOpts);
        expect(result.valid).toBe(false);
        expect(result.reason).toContain('TABLE_NOT_ALLOWED');
    });
    it('checks tables in subqueries', () => {
        const result = validateSQL('SELECT * FROM orders WHERE id IN (SELECT order_id FROM secret_data)', baseOpts);
        expect(result.valid).toBe(false);
        expect(result.reason).toContain('TABLE_NOT_ALLOWED');
    });
});
...
describe('validateSQL — dialect support', () => {
    ...
    it('SQLite fallback blocks non-SELECT in fallback mode', () => {
        ...
        const result = validateSQL('DELETE FROM orders WHERE id = 1', { ...baseOpts, dialect: 'SQLite' });
        expect(result.valid).toBe(false);
    });
});
// ─── stripSQLComments ────────────────────────────────────────────────────────
