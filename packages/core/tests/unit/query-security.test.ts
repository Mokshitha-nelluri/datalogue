...
DROP TABLE orders`;
    ...
    try {
      ...
      expect((err as DatalogueError).code).toBe('SQL_INJECTION_BLOCKED');
    }
  });
...
SELECT * FROM secret_data`;
    ...
    try {
      ...
      expect((err as DatalogueError).code).toBe('TABLE_NOT_ALLOWED');
    }
  });
...
DELETE FROM orders WHERE id = 1`;
    ...
    try {
      ...
      expect((err as DatalogueError).code).toBe('MUTATION_NOT_ALLOWED');
    }
  });
...
SELECT id, total FROM orders`;
    ...
    (ai.complete as ReturnType<typeof vi.fn>)
    ...
    const retryCall = (ai.complete as ReturnType<typeof vi.fn>).mock.calls[1];
...
SELECT id FROM orders`;
    ...
    try {
      ...
      expect((err as DatalogueError).code).toBe('SQL_EXECUTION_ERROR');
    }
  });
