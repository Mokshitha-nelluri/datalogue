...
function createMockAI(response: string = VALID_RESPONSE): AIProvider {
  return {
...
describe('Day 4 Integration — Multi-turn Context', () => {
  it('passes session history to AI on successive queries', async () => {
    ...
    const firstCall = (ai.complete as ReturnType<typeof vi.fn>).mock.calls[0];
    ...
    const secondCall = (ai.complete as ReturnType<typeof vi.fn>).mock.calls[1];
  ...
  it('does not share context across different sessions', async () => {
    ...
    const sess2Call = (ai.complete as ReturnType<typeof vi.fn>).mock.calls[1];
