...
describe('enforceAllowlist', () => {
  it('passes when all tables are allowed', () => {
    expect(() =>
      enforceAllowlist(['orders', 'customers'], ['orders', 'customers']),
    ).not.toThrow();
  });
  ...
  it('throws TABLE_NOT_ALLOWED for disallowed table', () => {
    ...
    ).toThrow(DatalogueError);

    try {
      enforceAllowlist(['orders', 'secret_data'], ['orders']);
    } catch (err) {
      expect((err as DatalogueError).code).toBe('TABLE_NOT_ALLOWED');
      expect((err as DatalogueError).message).toContain('secret_data');
    }
  });

  it('strips schema prefix before checking', () => {
    expect(() =>
      enforceAllowlist(['public.orders'], ['orders']),
    ).not.toThrow();
  });

  it('throws for schema-qualified table not in list', () => {
    expect(() =>
      enforceAllowlist(['pg_catalog.pg_tables'], ['orders']),
    ).toThrow(DatalogueError);
  });
